...
var all_text_phones = document.getElementsByClassName('phone_text_form');
for (jj = 0; jj < all_text_phones.length; jj++){
    mask(all_text_phones[jj]);
}
...
$('#myCarousel').find('.item:first').css('display', 'block');

setTimeout(function() {
  $('#myCarousel').find('.item:first').css('display', '').addClass('active');
}, 21);
...
$('#myCarousel').on('slid.bs.carousel', function() {
    currentIndex = $('#myCarousel .active').index() + 1;
...
(function ($) {
    ...
    function headerAfterScroll(){
            var sElement = $('.layout-header'),
                sDistace = window.innerHeight * .85,
                sFromTop = $(window).scrollTop();

            if (sElement.hasClass('blog')) {
              return false;
            }
...
(function ($) {
    ...
    if (!$('.layout-header').hasClass('blog')) {
      $("a.spy-link").on('click', function(event) {
         event.preventDefault();
         var hash = this.hash;

         if ($('.menu-icon').hasClass('active')) {
          $('.menu-icon').trigger('touchstart');
         }

         $('html, body').animate({
             scrollTop: $(window).width() > 1000 ? $(hash).offset().top - 120 : $(hash).offset().top - 40
    ...
    if ($('#how-it-works').length) {
      $(window).on('scroll.arrow', function() {
        var el = $('#how-it-works');

        if (inViewport(el[0], .5)) {
          el.find('.arrow-cont > div').addClass('show');
          $(window).off('scroll.arrow');
        }
      }).trigger('scroll.arrow');
    }

    if ($('#action-block').length) {
      $(window).on('scroll.arrow2', function() {
        var el = $('#action-block .ab-cards');

        if (inViewport(el[0], .5)) {
          el.find('.abc-arrow').addClass('show');
          $(window).off('scroll.arrow2');
        }
      }).trigger('scroll.arrow2');

      $(window).on('scroll.counter', function() {
        var el = $('#ab-counter');

        if (inViewport(el[0], .1)) {
          var options = {
            useEasing : true,
            useGrouping : false,
            separator : ',',
          ...
          var counter = new CountUp(el.find('span')[0], 0, el.find('span').data('count'), 0, 2.5, options);
          counter.start();
    ...
    var is_form_xhr = false,
        form_xhr,
        app_path = $('body').data('path'),
        modal_open,
        thank_you = false;
    ...
    //phone test
    var phone = form.find('.phone_text_form').val();
    if (phone.length!=14){
        return false;
    }
    ...
    //end phone test
      ...
      if (form[0].id == 'reg-form' || form[0].id == 'consult-form') {
        var form_action = app_path + "/ajax/registration",
          input_name = "registration";
      }
      else {
        var form_action = app_path + "/ajax/subscribe",
      ...
      form_xhr = $.ajax({type: "POST", url: form_action, data: form.serialize(), cache: false, dataType: "json",
        success: function(arr) {
          ...
          if (arr.error == true) {
            ...
            if (arr.errorCode != '') {
              form.find('.form-group:last').after('<div class="form-group error-msg"><div class="form-col form-col-100">' + arr.errorCode + '</div></div>');
            }

            for (var field in arr.errorFields) {
              form.find('[name="' + input_name + '[' + field + ']"]').each(function() {
                if (this.type == 'hidden') {
                  return;
                }
                else if (this.type == 'checkbox') {
                  $(this).addClass('error-field').parent()
                    .after('<div class="error-msg">' + arr.errorFields[field].join('<br>') + '</div>');
                }
                else {
                  if (form[0].id == 'subscribe') {
                    $(this).addClass('error-field').before('<div class="error-msg">' + arr.errorFields[field].join('<br>') + '</div>');
                  }
                  else {
                    $(this).addClass('error-field').after('<div class="error-msg">' + arr.errorFields[field].join('<br>') + '</div>');
                  }
                }
              });
            }
            ...
            if (form[0].id == 'subscribe') {
              form.html('<div class="success-msg">' + arr.msg + '</div>');
            }
            else {
              modal_open.modal('hide');
              thank_you = true;
              // form.prepend('<div class="success-msg form-group clearfix"><div class="form-col form-col-100">' + arr.msg + '</div></div>');
              // form.find('.form-group:not(:first)').hide();
            }
          }
        },
        error: function(jqXHR, textStatus, errorThrown) {
          ...
          form.find('.error-msg').remove();
          form.find('.form-group:last').after('<div class="error-msg form-group"><div class="form-col form-col-100">Request error! Try again later</div></div>');
    ...
    // modal events
    $('.order-modal').on('show.bs.modal', function() {
      modal_open = $(this);
    });
    ...
    $('.order-modal').on('hidden.bs.modal', function() {
      ...
      modal.find('input:not([type="hidden"])').removeClass('error-field').val('');
    ...
    var open_order_modal = 0;

    $('.program-btn').on('click', function(e) {
      e.preventDefault();

      $(this).parents('.modal').modal('hide');
      open_order_modal = $(this).data('target');
    });

    $('.details-modal').on('hidden.bs.modal', function() {
      console.log(open_order_modal);
      if (open_order_modal !== 0) {
        $(open_order_modal).modal('show');
        open_order_modal = 0;
      }
    });
    ...
    $('#phones-slider').slick({
      ...
      var that = this;
        ...
        setTimeout(function() {
          $(that).find('[data-slick-index="' + currentSlide + '"]').siblings().removeClass('show-messages');
        }, 21);
    }).on('afterChange', function(e, slick, currentSlide){
      var that = this;

      setTimeout(function() {
        $(that).find('[data-slick-index="' + currentSlide + '"]').addClass('show-messages');
      }, 250);
    });

    setTimeout(function() {
      $('#phones-slider').find('[data-slide="1"]').addClass('show-messages');
    }, 21);
...
$(document).ready(function(){
  $("#tabs-main .nav-tabs a").click(function(e){
    e.preventDefault();
    $(this).tab('show');
  });
    $("#tabs-main .accordion-content-title a").click(function(e){
    $("#tabs-main .accordion-content-title a").removeClass('aactive');
